...
- **Own your factory defaults** via `g_modsSettingsApi.setModDefaults(linkage, {varName: value})`: by default the API derives a mod's defaults (what the per-mod reset restores) from its registered template; call this to declare them explicitly instead, at any time. Once declared they are yours - no re-registration or `settingsVersion` bump overwrites them.
- **Re-announce highlighted options** via `g_modsSettingsApi.resetFeatureHighlights(linkage, varNames=None)`: clear the "already seen" state of options flagged with `markNew`, so they light up again (leaving beta, re-introducing a feature). Omit `varNames` to clear the whole mod; the per-control alternative is bumping `markNew`'s `token`.
...
- `g_modsSettingsApi`: `updateImage`, `updateImageAtlas`, `registerLiveSettingsChange` / `unregisterLiveSettingsChange`, `notifyLiveSettingsChange`, `reloadModTemplate`, `setModCollapsed`, `getVersion` / `getVersionTuple`, `registerModTranslation`, `setModDefaults`, `resetFeatureHighlights`, the `multiColumnTemplate` argument of `setModTemplate` / `reloadModTemplate` (guard with `try` / `except TypeError`)
